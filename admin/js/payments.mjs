...
async function enhanceOrderCard(orderInput) {
  ...
  panel.querySelector('[data-payment-terms]').addEventListener('change', async e => {
    const previous = order.paymentTerms || 'pay_on_delivery'; e.target.disabled = true;
    try { await setOrderPaymentTerms(orderId, e.target.value); } catch (err) { alert(err.message); e.target.value = previous; } finally { e.target.disabled = false; }
  });
  ...
  panel.querySelector('[data-pay-save]').addEventListener('click', async e => {
    ...
    try { await recordPayment(orderId, { amount, method, reference, note }); await refreshPaymentsForCard({ id: orderId }); }
    catch (err) { alert(err.message); e.currentTarget.disabled = false; e.currentTarget.textContent = 'Save Payment'; }
  });
}
...
export function initPaymentsAdmin() {
  ...
  return () => { if (observer) observer.disconnect(); if (authStop) authStop(); };
}
